const {advanceTime, createDao, sharePrice, remaining, GUILD, CoreContract, OnboardingContract, VotingContract, RagequitContract, ETH_TOKEN} = require('../../utils/DaoFactory.js');
...
contract('MolochV3 - Ragequit Adapter', async accounts => {
  ...
  it("should not be possible for a non DAO member to ragequit", async () => {
    ...
    const coreAddress = await dao.getAddress(sha3("core"));
    const core = await CoreContract.at(coreAddress);
    ...
    let guildBalance = await core.balanceOf(dao.address, GUILD, ETH_TOKEN);
